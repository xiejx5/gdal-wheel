...
def test_zstd(tmp_path):
    ...
    with gdal.GetDriverByName("GTiff").Create(
        path, 16, 16, 1, gdal.GDT_UInt16, ["COMPRESS=ZSTD"]
    ) as ds:
...
def test_bundled_data():
    ...
    assert (
        Path(gdal.GetConfigOption("GDAL_DATA")).resolve() == (root / "gdal").resolve()
    )
...
def test_explicit_paths_and_escape_hatch(tmp_path):
    ...
    subprocess.run(
        ...
        env={**os.environ, "GDAL_DATA": str(tmp_path), "PROJ_DATA": "explicit-proj"},
        check=True,
    )
    env = {
        k: v
        for k, v in os.environ.items()
        if k not in ("GDAL_DATA", "PROJ_DATA", "PROJ_LIB")
    }
    subprocess.run(
        [
            sys.executable,
            "-c",
            'import osgeo, os; assert "PROJ_DATA" not in os.environ',
        ],
        env={**env, "SKIP_GDAL_DATA_CHECK": "1"},
        check=True,
    )
...
def test_vsicurl(tmp_path):
    ...
    try:
        with gdal.Open(
            f"/vsicurl/http://127.0.0.1:{server.server_port}/remote.tif"
        ) as ds:
