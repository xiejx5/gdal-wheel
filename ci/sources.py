...
def download(record, destination):
    ...
    if not destination.exists() or hashlib.sha256(destination.read_bytes()).hexdigest() != digest:
        ...
        with urllib.request.urlopen(record['url'], timeout=180) as response, temporary.open('wb') as output:
            import shutil
...
def resolve(version=None):
    ...
    if version:
        ...
        release = max(stable, key=lambda r: tuple(map(int, r['tag_name'][1:].split('.'))))
    ...
    if not re.fullmatch('sha256:[0-9a-f]{64}', digest):
        raise ValueError('Official source asset has no SHA256 digest; refusing to build')
    ...
    return dict(version=version, tag=release['tag_name'], commit=commit,
