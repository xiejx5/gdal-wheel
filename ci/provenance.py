...
def write(prefix, lock, release):
    prefix = Path(prefix)
    components = [dict(type='library', name='GDAL', version=release['version'],
                       hashes=[{'alg':'SHA-256','content':release['source']['sha256']}],
                       externalReferences=[{'type':'distribution','url':release['source']['url']}])]
    if os.name == 'nt':
        receipt = prefix / 'vcpkg-packages.json'
        packages = json.loads(receipt.read_text(encoding='utf-8-sig'))
        for package in packages.values():
            components.append(dict(type='library', name=package['package_name'], version=package['version']))
    else:
        for name, dependency in lock['dependencies'].items():
            components.append(dict(type='library', name=name, version=dependency['version'],
    ...
    metadata = dict(os=platform.platform(), architecture=platform.machine(),
