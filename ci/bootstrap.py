...
def _setup_bundled_data():
    ...
    if ('GDAL_DATA' not in os.environ and _gdal.GetConfigOption('GDAL_DATA') is None
    ...
    if proj_data.is_dir() and not any(os.environ.get(k) for k in ('PROJ_DATA', 'PROJ_LIB')):
