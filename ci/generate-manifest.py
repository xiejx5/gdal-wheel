...
def generate(directory, results, release, repository):
    ...
    for path in sorted(directory.glob('*.whl')):
        ...
        if len(report) != 1 or any(report[0].get(k) != 'passed' for k in ('feature_tests', 'leak_tests')):
    ...
    manifest = dict(schema_version=1, complete=True, version=release['version'], upstream=release,
                    platforms={p:'passed' for p in sorted(PLATFORMS)}, wheels=rows)
    (directory / 'manifest.json').write_text(json.dumps(manifest, indent=2) + '\n')
    base = f'https://github.com/{repository}/releases/download/gdal-v{release["version"]}'
    links = [f'<a href="{base}/{quote(r["filename"])}#sha256={r["sha256"]}">{html.escape(r["filename"])}</a><br>' for r in rows]
...
if __name__ == '__main__':
    ...
    generate(args.wheels, args.results, json.loads(args.release.read_text()), args.repository)
