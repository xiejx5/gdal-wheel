...
LINUX_SYSTEM = set(
    "libc.so.6 libm.so.6 libdl.so.2 librt.so.1 libpthread.so.0 libutil.so.1 libgcc_s.so.1 libstdc++.so.6 libresolv.so.2 ld-linux-x86-64.so.2".split()
)
WINDOWS_SYSTEM = set(
        ...
        if sys.platform == "linux":
            ...
            for path in files:
                ...
                for name, target in re.findall(r"^\s*(\S+) => (\S+)", linked, re.M):
                    if name not in LINUX_SYSTEM and not Path(
                        target
                    ).resolve().is_relative_to(root):
                        raise RuntimeError(
                            f"External native dependency: {name} => {target}"
                        )
            ...
            for path in files:
                ...
                for entry in getattr(binary, "DIRECTORY_ENTRY_IMPORT", []) + getattr(
                    ...
                    if (
                        name in bundled
                        or name in WINDOWS_SYSTEM
                        or name.startswith(("api-ms-win-", "ext-ms-win-"))
                        or re.fullmatch(r"python3\d*\.dll", name)
                    ):
                        continue
